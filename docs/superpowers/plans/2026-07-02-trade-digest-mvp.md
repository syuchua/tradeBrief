...
def run(session: str, today: date) -> None:
    ...
    fetch_etf_quotes(watchlist_codes)  # warms up watchlist quotes for future dashboard use
    ...
    payload = build_payload(market_overview, sector_flow, macro_updates, news_items, tactical_positions, dca_due)
