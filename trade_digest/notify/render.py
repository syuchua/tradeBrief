# trade_digest/notify/render.py
"""交易简报 HTML 渲染 —— 将结构化数据渲染为响应式 HTML 报告。

本模块只负责内容生成，不涉及任何通知渠道。邮件、飞书、Telegram、HTML 导出
都共用同一个 render_report() 输出。
"""
...
def _html_document(title: str, body: str) -> str:
    """将 body 内容包裹为完整的 HTML 文档（兼容邮件客户端和独立浏览器）。"""
...
def render_report(
    ...
    """将结构化数据渲染为完整的 HTML 交易简报。

    不依赖任何通知渠道；输出可直接用于邮件、飞书卡片、文件导出等。
    """
...
# 向后兼容：旧代码中 import render_email 仍然可用（emailer.py 会重新导出）
render_email = render_report
